...
resource "rancher2_machine_config_v2" "two-config" {
  generate_name = "two-config"
  amazonec2_config {
...
resource "rancher2_cluster_v2" "two-rke2" {
  name = "two"
  kubernetes_version = 	"v1.26.12+rke2r1"
  ...
  rke_config {
    machine_pools {
      ...
      quantity = 1
      machine_config {
        kind = rancher2_machine_config_v2.two-config.kind
        name = rancher2_machine_config_v2.two-config.name
      }
    }
  }
}